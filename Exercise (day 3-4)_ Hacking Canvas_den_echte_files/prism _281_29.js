...
const barbie = "./barbieQuiz.json";
const barbieClass = document.querySelector(".team-barbie");
const iframe = document.createElement("iframe");
iframe.src =
	"https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/237408858&color=%23ecc1d3&auto_play=true&hide_related=false&show_comments=false&show_user=false&show_reposts=false&show_teaser=false&visual=false";
iframe.allow = "autoplay";

const musicButton = document.createElement("button");
musicButton.innerHTML = "Are you a BARBIE GIRL?".bold();
...
journalDiv.id = "journalDiv";

const journalButton = document.createElement("button");
journalButton.id = "journalButton";

init();
function init() {
	barbieClass.insertAdjacentElement("afterbegin", musicButton);
	musicButton.addEventListener("click", music);
	barbieClass.insertAdjacentElement("afterbegin", journalDiv);
	notepad();
	getData(barbie);
}

function music() {
	barbieClass.insertAdjacentElement("beforeend", quizArea);
	document.getElementById("quizArea").scrollIntoView({
		behavior: "smooth",
	});
	musicButton.replaceWith(iframe);
...
async function getData(link) {
	...
	//   questions and answers in quiz array
	for (const e of data.questions) {
		{
			let p = new Question((_question = e.question), (_answers = e.answers));
			quiz.push(p);
		}
	}
}
...
function quizzing() {
	...
	const targetArea = document.getElementById("quizArea");

	showQuestion(current);

	function showQuestion(i) {
		targetArea.innerHTML = `<h4>Which Barbie are you?</h4><br>`;
		targetArea.innerHTML += quiz[i]._question;
		for (a of quiz[i]._answers) {
			...
			btn.addEventListener("click", function () {
				for (const [char, points] of Object.entries(a.scores)) {
					qScore[char] += points;
				}
				current++;
...
function notepad() {
	...
	noteTab.contentEditable = true;
	noteTab.classList.add("noBorder");

	let noteWindow = window.open("", "", "width=600, height=400");
	noteWindow.document.body.appendChild(noteTab);
	noteTab.focus();

	let style = noteWindow.document.createElement("style");
	style.textContent = `
    html, body {
      margin: 0;
      padding: 0;
  ...
  `;
	...
	journalDiv.insertAdjacentElement("afterbegin", journalButton);
...
document.addEventListener("DOMContentLoaded", () => {
	const elements = document.querySelectorAll(".team-3_day-3-4");
	elements.forEach((el) => {
		el.innerText = textToMorse(el.innerText);
	});
});
